  ...
  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[150] flex items-center justify-center p-4">
      <div className="bg-white rounded-[40px] w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="p-8 bg-indigo-700 text-white flex items-center justify-between">
          <div className="flex items-center gap-4">
            <UserPlus size={28} />
            <div>
              <h2 className="text-xl font-black uppercase tracking-tight">Pendaftaran Ahli Baru</h2>
              <p className="text-indigo-200 text-[10px] font-bold uppercase tracking-widest">Daftar 1-4 Unit Sekaligus</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              ...
              <input required value={name} onChange={e => setName(e.target.value)} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-black text-slate-900 text-sm outline-none focus:ring-4 focus:ring-indigo-600/5 uppercase" placeholder="NAMA MURID" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Kelas</label>
              <input required value={className} onChange={e => setClassName(e.target.value)} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-black text-slate-900 text-sm outline-none focus:ring-4 focus:ring-indigo-600/5 uppercase" placeholder="KELAS" />
            </div>
          </div>

          <div className="space-y-4 pt-4 border-t border-slate-100">
            <h3 className="text-xs font-black uppercase text-slate-900 tracking-widest flex items-center gap-2 mb-4">
              ...
              {categories.map(cat => (
                <div key={cat.id} className="space-y-1.5">
                  ...
                  <select 
                    ...
                    className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-xl font-bold text-[11px] text-slate-800 outline-none focus:bg-white transition-all uppercase"
                  >
          ...
          <div className="flex gap-4 pt-4">
            <button type="button" onClick={onClose} className="flex-1 py-5 bg-slate-100 text-slate-600 rounded-[24px] font-black text-xs uppercase tracking-widest">Batal</button>
            <button type="submit" className="flex-[2] py-5 bg-indigo-600 text-white rounded-[24px] font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 flex items-center justify-center gap-3 hover:bg-indigo-700 transition-all">
              <CheckCircle2 size={20} /> Sah & Daftar Ahli
            </button>
          </div>
        </form>
      </div>
